...
const MyHotels = () =>{
    ...
    const { data:hotelData, isError } = useQuery({ queryKey: ["fetchMyHotels"], queryFn: apiClient.fetchMyHotels, retry: false});
                ...
                {hotelData.map((hotel)=>(
                    <div key={hotel._id} className="flex flex-col justify-between border border-slate-300 rounded-lg p-8 gap-5">
                        ...
                        <span className="flex justify-end">
                            <Link to={`/edit-hotel/${hotel._id}`} className="flex bg-blue-600 text-white text-xl font-bold p-2 hover:bg-blue-500">View Deatils</Link>
                        </span>
                    </div>
                ))}
            </div>
